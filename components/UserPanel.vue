<template>
  <div class="wrapper overflow-auto relative h-screen" :class="wrapperClass">
    <div>
      ...
      <div class="flex flex-col justify-center mt-[90px]">
        ...
        <div class="flex flex-col w-full mt-[12px]">
          <div v-for="user in agents" :key="user.id" class="flex w-full items-center justify-center" :class="userStyle">
            <div>
              ...
              <div v-else class="flex-col items-center flex" :class="{ 'ml-4 w-[144px]': showUsers }">
                <div class="bg-greenDark relative left-[31px] top-[27px] border-greenDark h-4 w-4 rounded-full" />
                <div class="w-[65px] h-[65px] my-4 border bg-orangeLight border-orangeLight rounded-2xl flex items-center justify-center">
                  <span class="tracking-wide text-orangeDark text-4xl">
                    {{ formatName(user.name) }}
                  </span>
                </div>
